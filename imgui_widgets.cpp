...
bool ImGui::ROffice_CollapseButton(const char* str_id)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    float height = GetFrameHeight();
    return ROffice_CollapseButton_(window->GetID(str_id), {window->DC.CursorPos.x, window->DC.CursorPos.y + (height - g.FontSize) * 0.5f});
}

bool ImGui::ROffice_CloseButton(const char* str_id)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    float height = GetFrameHeight();
    return CloseButton(window->GetID(str_id), {window->DC.CursorPos.x, window->DC.CursorPos.y + (height - g.FontSize) * 0.5f});
}
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    RenderArrowNoFill(window->DrawList, bb.Min, text_col, window->Collapsed ? ImGuiDir_Right : ImGuiDir_Down, 1.0f, 1.0f);
...
// Button to collapse from mdi maximum
bool ImGui::ROffice_ExpandButton_(ImGuiID id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
// Button to collapse from mdi maximum restore
bool ImGui::ROffice_CollapseButton_(ImGuiID id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
